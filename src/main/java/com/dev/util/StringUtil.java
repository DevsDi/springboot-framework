...
public class StringUtil
{
	...
	{
		return (objValue == null ? "" : objValue.toString().trim());
	}
	/**
...
//		s = Replace(s, " ", "&nbsp;");
	...
	*/
	 ...
	 * @throws
	 * @author vino     
	 * @date Jan 27, 2015 4:07:01 PM  
	 * @version V1.0
	 */
	public static boolean isNotEmptyList(Object list){
...
//
	 ...
	 * @return
	 */
	public static String replaceSpecStr(String orgStr,String mark){  
	    if (null!=orgStr&&!"".equals(orgStr.trim())) {  
	        String regEx="[\\s~·`!！@#￥$%^……&*（()）\\-——\\-_=+【\\[\\]】｛{}｝\\|、\\\\；;：:‘'“”\"，,《<。.》>、/？?]";  
	        Pattern p = Pattern.compile(regEx);  
	        Matcher m = p.matcher(orgStr);  
	        return m.replaceAll(mark);  
	    }  
	    return null;  
	} 
	
	public static String clearSearchStr(String str){
		if(!isEmpty(str)){
			String regEx="'"; 
			Pattern p = Pattern.compile(regEx); 
	...
	// 校验字符串是否存在字符串集中
	public static boolean validateStr(String fileName) {
		
		String[] fileTypeArray = new String[] { "doc", "docx", "xls", "xlsx","ppt","pptx","pdf","jpeg", "jpg", "png", "txt","tiff", "gif"};
		try {
			String fileType = fileName.substring(fileName.lastIndexOf(".") + 1);
			
			for (String file : fileTypeArray) {
	...
	public static String getStateCode(Date rent_enddate) {
		long day = DateUtil.getDaysBetween(new Date(), rent_enddate);
		String state_code = "INRENT";
		if (day >= 0 && day <= 30) {
			state_code = "NEARING";
		} else if (day < 0) {
			state_code = "EXPIRE";
    ...
    public static String processDept(String department) {
